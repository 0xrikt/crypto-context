...
> 最后更新：2026-09-06 ·（**本文件在公开仓库中——禁止写入任何真实余额/账户/密钥**）
>
> 这是给下一个研发的单一入口：读完即可接手。代码与文档全部在 `app/` 子目录（独立 git 仓库，push `main` 即 Vercel 自动部署）。

> **2026-09-06 收尾状态**：Supabase 原项目因 paused 导致认证不可达，已确认恢复至 Healthy。免费方案有低数据库活跃度自动暂停机制，但未读取此次暂停通知确认具体触发者。
>
> **已上线**：每 3 天更新独立保活表的一条时间记录，Vercel 调度已启用。代码提交 `9bddf11`，生产最新提交 `4914988`，部署 `dpl_4T14okZ3Gf8RwT8pUx1dY4BWFtSo` 为 READY。真实写入及独立回读通过；无密钥端点请求与匿名表访问均返回 401。详见 [保活验收证据](docs/keepalive.md)。下次数据库写入日为 2026-09-09，北京时间 10:00 左右；尚未观察到自动调度执行，不承诺免费保活永不暂停。
>
> **仅本地保留**：`fix/auth-service-unavailable` 分支保存登录、注册将后端连接/5xx 故障误报为凭据错误的修复和 8 项回归测试；未推送、未部署。范围内全部 76 测试、类型检查、相关 ESLint 和生产构建通过。此分支也包含本次收尾文档，不要直接视作生产 main。
>
> **仍未验收**：真实账号成功登录、注册邮件送达及确认回调。当前仅确认认证健康接口 200、注册开启、邮箱确认开启，以及无效凭据/邮箱的校验响应。
>
> **接续**：若继续认证补丁，先按上述分支恢复上下文，完成真实认证验收并取得发布授权后再合入 main。保活无需本机或本会话保持运行。Vercel CLI 登录已恢复；Supabase 本地管理 token 仍失效，管理页使用 Chrome 的 reeze112（工作）配置。临时 CRON_SECRET 文件已删除，生产密钥保存在 Vercel，未入库。

---

## 1. 状态与入口

**当前状态见文首 2026-09-06 收尾记录。** 下文功能与架构说明主要保留自 6 月版本，不作为本次真实认证流程验收证据。
